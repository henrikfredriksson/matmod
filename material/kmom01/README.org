...
En tidsserie är en följd observationer ~y_1, y_2,...,y_T~ som är ordnade
...
Med hjälp av [[https://matplotlib.org/][matplotlib]] så kan vi plotta en tidsseri.
...
Antag att punkterna i grafen ovan representerar en funktion =f= som är
temperaturen under en 23 timmars period. Tyvärr så har vi inte
tillgång till funktionen och =f= som matematiskt uttryck och vi kan
därför inte använda derivatans definition för att för att beräkna
...
enkelt inte hur funktionen =f= är definierad. Vi kan däremot använda oss
av en numerisk approximation till derivering som i boken kallas för
differenskvoter. Fördelen med denna approximation är att den kan
